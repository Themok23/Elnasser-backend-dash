...
@section('title', translate('Create Story'))
...
<div class="content container-fluid">
    <div class="page-header">
        <h1 class="page-header-title">
            ...
            <span>
                {{ translate('messages.create_story') }}
            </span>
        </h1>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.stories.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-control" value="{{ old('title') }}" placeholder="{{ translate('messages.enter_story_title') }}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="type" class="form-control" required id="story_type">
                        <option value="image" {{ old('type') == 'image' ? 'selected' : '' }}>{{ translate('messages.image') }}</option>
                        <option value="video" {{ old('type') == 'video' ? 'selected' : '' }}>{{ translate('messages.video') }}</option>
                    </select>
                </div>

                <div class="form-group">
                    <label>{{ translate('messages.media') }} <span class="text-danger">*</span></label>
                    <input type="file" name="media" class="form-control" required accept="image/*,video/*" id="media_input">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" rows="3" placeholder="{{ translate('messages.enter_story_description') }}">{{ old('description') }}</textarea>
                </div>
                ...
                <button class="btn btn-primary" type="submit">{{ translate('messages.create') }}</button>
            </form>
        </div>
    </div>
</div>
    ...
    document.getElementById('media_input').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                if (type === 'image') {
                    preview.innerHTML = '<img src="' + e.target.result + '" style="max-width: 300px; max-height: 300px; border-radius: 4px;">';
                } else {
                    preview.innerHTML = '<video src="' + e.target.result + '" controls style="max-width: 300px; max-height: 300px; border-radius: 4px;"></video>';
                }
            };
