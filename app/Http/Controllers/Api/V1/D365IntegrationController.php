...
class D365IntegrationController extends Controller
{
    ...
    public function addPoints(Request $request)
    {
        ...
        try {
            ...
            // Queue sync to D365 (non-blocking, will retry if fails)
            // This ensures data is not lost if D365 API is temporarily unavailable
            try {
                $dynamicsService = new \App\Services\Dynamics365Service();
                if ($dynamicsService->isEnabled()) {
                    // Queue loyalty points sync - preserves data even if D365 is down
                    $dynamicsService->updateLoyaltyPoints($customer, true);
                }
            } catch (\Exception $e) {
                // Log but don't fail - points are already added in Laravel
                Log::warning('Failed to queue D365 points sync', [
                    'customer_id' => $customer->id,
                    'error' => $e->getMessage()
                ]);
            }
